...
bool coincidencias(const char *nombre1, const char *nombre2,
		   const char *nombre3);
...
void asignar_resultado(RESULTADO_ATAQUE *resultado, const ataque_t *ataque,
		       pokemon_t *oponente_pkm);

bool jugada_disponible(abb_t *disponibles, jugada_t jugada_jugador);

void asignar_puntaje(int *puntaje, const ataque_t *ataque,
		     RESULTADO_ATAQUE efectividad);
...
juego_t *juego_crear()
{
	...
	juego->jugadores[JUGADOR1].jugadas_disponibles =
		abb_crear(comparador_abb);
	juego->jugadores[JUGADOR2].jugadas_disponibles =
		abb_crear(comparador_abb);
...
JUEGO_ESTADO juego_cargar_pokemon(juego_t *juego, char *archivo)
{
	...
	con_cada_pokemon(juego->info_pokemones, insertar_en_lista,
				       ...
				       const char *nombre3)
{
	...
	if (jugador == JUGADOR1) {
		juego->jugadores[JUGADOR2].sus_pokemones = lista_insertar(
			juego->jugadores[JUGADOR2].sus_pokemones, pkm3);
	} else
		juego->jugadores[JUGADOR1].sus_pokemones = lista_insertar(
			juego->jugadores[JUGADOR1].sus_pokemones, pkm3);

	asignar_jugadas_disponibles(
		juego->jugadores[jugador].jugadas_disponibles, pkm1);
	asignar_jugadas_disponibles(
		juego->jugadores[jugador].jugadas_disponibles, pkm2);

	if (jugador == JUGADOR1) {
		asignar_jugadas_disponibles(
			juego->jugadores[JUGADOR2].jugadas_disponibles, pkm3);

	} else
		asignar_jugadas_disponibles(
			juego->jugadores[JUGADOR1].jugadas_disponibles, pkm3);
	;
				     ...
				     jugada_t jugada_jugador2)
{
	...
	if (!jugada_disponible(juego->jugadores[JUGADOR1].jugadas_disponibles,
			       jugada_jugador1)) {
		return resultado;
	}

	if (!jugada_disponible(juego->jugadores[JUGADOR2].jugadas_disponibles,
			       jugada_jugador2)) {
		return resultado;
	}

	//saca los pokemones en sus mochilas , si es q existen
	pokemon_t *pkm1 = lista_buscar_elemento(
		juego->jugadores[JUGADOR1].sus_pokemones, comparar_nombres,
		jugada_jugador1.pokemon);
	pokemon_t *pkm2 = lista_buscar_elemento(
		juego->jugadores[JUGADOR2].sus_pokemones, comparar_nombres,
	...
	const ataque_t *atk1 =
		pokemon_buscar_ataque(pkm1, jugada_jugador1.ataque);
	const ataque_t *atk2 =
		pokemon_buscar_ataque(pkm2, jugada_jugador2.ataque);
	...
	asignar_puntaje(&juego->jugadores[JUGADOR1].puntaje, atk1,
			resultado.jugador1);

	asignar_puntaje(&juego->jugadores[JUGADOR2].puntaje, atk2,
			resultado.jugador2);
...
{
	arbol_disponibles =
		abb_insertar(arbol_disponibles, (char *)ataque->nombre);
}

void asignar_jugadas_disponibles(abb_t *jugadas_disponibles, pokemon_t *pkm1)
{
	jugadas_disponibles =
		abb_insertar(jugadas_disponibles, (char *)pokemon_nombre(pkm1));
...
float redondear(float numero)
{
	...
	float parte_decimal =
		numero - (float)entero; // Obtenemos la parte decimal

	if (parte_decimal > 0) {
		return (float)entero +
		       1; // Si hay parte decimal, redondeamos hacia arriba sumando 1 a la parte entera
	} else {
...
bool jugada_disponible(abb_t *disponibles, jugada_t jugada_jugador)
{
	if (abb_quitar(disponibles, jugada_jugador.ataque) == NULL &&
	    abb_quitar(disponibles, jugada_jugador.pokemon) == NULL)
		return false;
