...
jugada_t elige_jugada_j1(menu_t *menu, juego_t *juego, char *p1, char *p2,
			 char *p3);

JUEGO_ESTADO precompetencia(menu_t *menu, juego_t *juego,
			    elecciones_t *jugador1, adversario_t *adversario,
			    char *eleccion_adversario1,
			    char *eleccion_adversario2,
			    char *eleccion_adversario3);
JUEGO_ESTADO competencia(menu_t *menu, juego_t *juego, elecciones_t *jugador1,
			 adversario_t *rival);
void delay(int d);
...
int main(int argc, char *argv[])
{
	...
	estado = menu_agregar_comando(nueva_partida, "c", "Cargar archivo",
				      pedir_archivo);

	estado = menu_agregar_comando(nueva_partida, "v", "Ver pokemones",
	...
	estado = menu_agregar_comando(nueva_partida, "i",
				      "Ver informacion del juego",
	...
	while (!juego_finalizado(juego) && estado != MENU_SALIR) {
		...
		estado = menu_ejecutar_comando(nueva_partida, linea,
					       nueva_partida);
	}
...
bool pedir_archivo(void *menu)
{
	...
	estado = juego_cargar_pokemon((juego_t *)entregar_app(menu),
				      entregar_nombre_archivo(menu));
...
bool mostrar_pokemon_disponibles(void *menu)
{
	...
	lista_t *lista_pkm =
		juego_listar_pokemon((juego_t *)entregar_app(menu));
	...
	do {
		*posiciones = 0;
		lista_con_cada_elemento(lista_pkm, imprimir_nombre_pkm,
					posiciones);
		...
		while (pkm_eleccion >= lista_tamanio(lista_pkm) &&
		       pkm_eleccion != -1) {
		...
		if (pkm_eleccion != -1) {
			pokemon_t *a_observar_pkm = lista_elemento_en_posicion(
				lista_pkm, (size_t)pkm_eleccion);
...
bool informacion_pokemones(void *menu)
{
	...
	if (!entregar_nombre_archivo(menuu) ||
	    lista_vacia(juego_listar_pokemon(juego))) {
		puts(LIMPIAR_PANTALLA);
	...
	case 'r':
		...
		break;
	case 'a':
		lista_con_cada_elemento(lista_pkm, imprimir_info_pkm,
					posiciones);
...
bool jugar(void *menu)
{
	...
	if (menu == NULL || !entregar_app(menu) ||
	    lista_vacia(juego_listar_pokemon(entregar_app(menu_)))) {
	...
	adversario_t *rival = adversario_crear(
		juego_listar_pokemon((juego_t *)juego)); //TODO_OK = 0 ,

	char *eleccion_adversario1 = NULL, *eleccion_adversario2 = NULL,
	     *eleccion_adversario3 = NULL;
	...
	estado = precompetencia(menu, juego, jugador1_e, rival,
				eleccion_adversario1, eleccion_adversario2,
			    ...
			    char *eleccion_adversario3)
{
	...
	lista_con_cada_elemento(juego_listar_pokemon(juego),
				imprimir_nombre_pkm, posiciones);
	...
	JUEGO_ESTADO estado = juego_seleccionar_pokemon(
		juego, JUGADOR1, jugador1->eleccionJugador1,
		jugador1->eleccionJugador2, jugador1->eleccionJugador3);
	...
	adversario_seleccionar_pokemon(adversario, &eleccion_adversario1,
				       &eleccion_adversario2,
				       &eleccion_adversario3);

	//Aca le guarda en juego_t  e intercambia
	estado = juego_seleccionar_pokemon(juego, JUGADOR2,
					   jugador1->eleccionJugador1,
					   jugador1->eleccionJugador2,
					   jugador1->eleccionJugador3);

	//Aca le guardo al adversario el tercero q le ortorga el primero
	adversario_pokemon_seleccionado(adversario, jugador1->eleccionJugador1,
					jugador1->eleccionJugador2,
					jugador1->eleccionJugador3);
...
JUEGO_ESTADO competencia(menu_t *menu, juego_t *juego,
			 elecciones_t *elecciones_j1, adversario_t *rival)
{
	...
	while (!juego_finalizado(juego) && ronda_n < 9) {
		...
		jugada_t juagada1 = elige_jugada_j1(
			menu, juego, elecciones_j1->eleccionJugador1,
			elecciones_j1->eleccionJugador2,
			elecciones_j1->eleccionJugador3);
		...
		resultado_ronda =
			juego_jugar_turno(juego, juagada1, jugada_adversario);
			 ...
			 char *p3)
{
	...
	pokemon_t *pkm_elegido = lista_buscar_elemento(
		pkm_disponibles, comparar_nombres_en_juego,
		jugada_actual.pokemon);
	...
	const struct ataque *ataque_elegido =
		pokemon_buscar_ataque(pkm_elegido, (const char *)buffer);
...
void jugador_seleccionar_pokemon(elecciones_t *elecciones)
{
	...
	elecciones->eleccionJugador1 = (char *)pokemon_nombre(
		lista_elemento_en_posicion(lista_pkm, (size_t)pkm1_p));

	dibujar_pkm(elecciones->eleccionJugador1);
	delay(TIEMPO_DELAY2);

	elecciones->eleccionJugador2 = (char *)pokemon_nombre(
		lista_elemento_en_posicion(lista_pkm, (size_t)pkm2_p));

	dibujar_pkm(elecciones->eleccionJugador2);
	delay(TIEMPO_DELAY2);

	elecciones->eleccionJugador3 = (char *)pokemon_nombre(
		lista_elemento_en_posicion(lista_pkm, (size_t)pkm3_p));
}
...
bool imprimir_info_pkm(void *pokemon, void *posicion)
{
	...
	printf("\n [%d]: %s     tipo: %s\n", *(int *)posicion,
	       pokemon_nombre(pkm), tipos[pokemon_tipo(pkm)]);
